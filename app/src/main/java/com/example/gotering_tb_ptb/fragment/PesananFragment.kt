...
class PesananFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        for (product in productsList) {
            ...
            tombolkurang.setOnClickListener {
                ...
                if (jumlah > 1) {
                    ...
                    val productsList = loadProducts()
                    val bookmarkStatusMap =
                        productsList.associateBy({ it.productName }, { true }).toMutableMap()

                    val bookmarkedProduct = Product(
                        product.productName,
                    ...
                    val sharedPreferences =
                        requireContext().getSharedPreferences("products", Context.MODE_PRIVATE)
            ...
            val productPrice =
                product.productPrice.toIntOrNull() ?: 0 // Assuming productPrice is a String
        ...
        checkout_button.setOnClickListener {
            ...
            if (orderName.isNotEmpty() && orderDeliver.isNotEmpty() && phoneNumber.text.toString()
                    .isNotEmpty()
            ) {
                ...
                for ((index, product) in productsList.withIndex()) {
                    orderType += "Product ${index + 1}:\nNama Pesanan: ${product.productName}\nJumlah: ${product.productJumlah}\nHarga: ${product.productPrice}\n\n"
                ...
                val message = "Harap masukkan nama, alamat tujuan, dan nomer hp!"
    ...
    private fun saveBookmarks(productsList: MutableList<Product>) {
        ...
        if (existingJsonString != null) {
            val existingBookmarks =
                Gson().fromJson(existingJsonString, Array<Product>::class.java).toMutableList()
            for (newBookmark in productsList) {
                val existingBookmarkIndex =
                    existingBookmarks.indexOfFirst { it.productName == newBookmark.productName }
