...
import com.example.gotering_tb_ptb.NOTIFICATION_CHANNEL_ID
import com.example.gotering_tb_ptb.NOTIF_CONTENT
import com.example.gotering_tb_ptb.NOTIF_ID
import com.example.gotering_tb_ptb.NOTIF_TITLE
import com.example.gotering_tb_ptb.NOTIF_UNIQUE_WORK
import com.example.gotering_tb_ptb.NotificationWorker
...
class TransaksiActivity : AppCompatActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val orderStatus: Int = intent.getIntExtra(KonfirmasiActivity.EXTRA_ORDER_STATUS, -1)
        ...
        for (order in ordersList) {
            ...
            val items = listOf("Pesanan Sedang dibuat.", "Pesanan Selesai.")

            val spinner: AutoCompleteTextView = findViewById(R.id.spinner_status)
            ...
            if (orderStatus != -1) spinner.setText(items[orderStatus], false)
            ...
            spinner.setOnItemClickListener { parent, view, position, id ->
                ...
                val oneTimeWorkRequest =
                    OneTimeWorkRequest.Builder(NotificationWorker::class.java)
                        .setInputData(
                            Data.Builder()
                                .putString(NOTIF_TITLE, orderID)
                                .putString(
                                    NOTIFICATION_CHANNEL_ID,
                                    "Gotering App"
                                )
                                .putInt(NOTIF_ID, orderID.toInt())
                                .putString(NOTIF_CONTENT, selectedItem)
                                .build()
                        )
                        .setConstraints(
                            Constraints.Builder()
                ...
                if (ordersList.isNotEmpty()) {
                    ordersList.find { it.orderID == orderID }?.let {
                        it.orderStatus = position
                    }
                }
